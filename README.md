...
- ✉️ 회원가입 이메일 인증 — 6자리 코드로 실제 사용 중인 이메일인지 확인해야 계정이 생성됨(카카오 가입은 제외)
...
### 사용자 (Users)
- `POST /api/v1/users/users/request_email_verification/` - 회원가입 이메일 인증 코드 요청 (이미 가입된 이메일이면 400, 60초 내 재요청 429)
- `POST /api/v1/users/users/confirm_email_verification/` - 이메일 인증 코드 확인 (성공 시 1시간 동안 회원가입 가능)
- `POST /api/v1/users/users/` - 회원가입 (닉네임/이메일/생년월일/비밀번호, 만 19세 미만이면 400, 이메일 인증 미완료면 400)
