...
- 매 항목마다 회귀 테스트 추가 후 전체 스위트 재실행 — 백엔드 300→307개, 프론트 22→27개까지 순증하며 전부 통과 유지. `check-all.sh`(포맷팅/린트/Django check)·`tsc`·`oxlint` 매 커밋마다 통과 확인. 커밋 10개로 분리(항목당 1개), 아직 PR 전 — 다음에 올릴 것.
...
전체 백엔드 테스트 스위트 307개(코드 품질 세션에서 추가된 카카오 이메일 검증 3개 + N+1 방지 쿼리카운트 1개 + SMTP 예외 처리 1개 + 스태프 권한 가드 1개, 이메일 인증 11개 + 생년월일 잠금 2개 + 온보딩 재진입 로직 2개 + 성격 upsert 3개, 유저 자유게시판 36개, 문의/신고하기 23개, 카카오 소셜 로그인/가입 11개 포함) 통과. 프론트엔드 27개(코드 품질 세션에서 추가된 ConfirmButton 2개 + usePaginatedList 5개 포함) 통과.
