...
### 추가 작업: 이메일 알림 기능 ✅ 완료
**목적:** 비동기적으로 발생하는 이벤트(연결 요청)를 사용자가 API를 다시 확인하지 않아도 알 수 있도록 이메일 알림 추가. (매칭 결과는 요청 즉시 동기 처리되어 API 응답에 바로 포함되므로 별도 알림이 불필요해 대상에서 제외)

**작업 내용:**
- [x] `apps/matching/notifications.py` 신규 — 알림 발송 로직을 뷰에서 분리
  - `notify_connection_requested` — 연결 요청 생성 시 받는 사람에게 발송
  - `notify_connection_accepted` — 연결 요청 수락 시 보낸 사람에게 발송 (거절/차단은 발송하지 않음)
  - 이메일 발송 실패(SMTP 오류 등)가 연결 생성/응답이라는 주 작업 자체를 실패시키지 않도록 `try/except` + 로깅으로 격리
- [x] `ConnectionViewSet.perform_create` / `respond` 액션에 연결

**검증:** `apps/matching/tests/test_connections_api.py`에 테스트 4개 추가(생성 시 수신자에게 발송, 수락 시 발신자에게 발송, 거절 시 미발송, 이메일 발송 자체가 실패해도 연결 생성은 201로 성공) + 로컬 개발 서버에서 두 계정으로 실제 회원가입 → 연결 요청 → 수락까지 진행해 콘솔에 출력된 두 이메일(수신자용 요청 알림, 발신자용 수락 알림) 내용 확인.
