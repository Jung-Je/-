...
- 커밋 상태: 문의/신고하기(`apps/support` 신규, 관리자 답변 기능 포함) + 카카오 소셜 로그인/가입 후속 조치 + README 정리까지는 PR #7로 `origin/main`에 이미 머지 완료(PR #6에 이어). **유저 자유게시판(`apps/board` 신규) 3커밋은 로컬 `feature` 브랜치에만 있고 아직 푸시 전** — 다음에 PR로 올릴 것
