...
전체 백엔드 테스트 스위트 290개(온보딩 재진입 로직 2개 + 성격 upsert 3개, 유저 자유게시판 36개, 문의/신고하기 23개, 카카오 소셜 로그인/가입 11개 포함) 통과.
...
- 커밋 상태: 문의/신고하기(`apps/support` 신규, 관리자 답변 기능 포함) + 카카오 소셜 로그인/가입 후속 조치 + README 정리까지는 PR #7로 `origin/main`에 이미 머지 완료(PR #6에 이어). **유저 자유게시판(`apps/board` 신규) + 성격 단계 500 버그 수정 + 온보딩 재진입 로직/설정 관심사 편집(+ 문서 업데이트) + has_completed_onboarding 백필 버그 수정 + 온보딩 성별/생년월일 필수 선택 강제, 총 10커밋은 로컬 `feature` 브랜치에만 있고 아직 푸시 전** — 다음에 PR로 올릴 것
...
- [x] 온보딩("내 카드 만들기") 3단계 마법사 (`features/onboarding/`) — 프로필(성별/생년월일 필수, 지역/자기소개) → 성격(MBTI/성향 3개/가치관, 전부 선택) → 관심사(카테고리별 선택, 1개 이상 필수) → `check_profile_completion` 호출. 프로필 단계는 `<form noValidate>`라 `required` 속성만으론 실제로 안 막혀서(커스텀 에러 UI를 쓰려고 브라우저 기본 검증을 꺼둔 것) `handleSubmit`에서 성별·생년월일을 직접 체크하도록 보완 — 설정 화면 프로필 편집은 같은 엔드포인트를 쓰지만 성별 "선택 안함"을 의도적으로 허용해서 그대로 둠. 매칭 가중치 3색 체계(관심사=코랄 50%·성격=바이올렛 30%·위치=틸 20%, DESIGN.md)를 각 단계 카드 윗선 색에 반영. `User.has_completed_onboarding`(한 방향 래치) 값이 true인 유저는 마법사 자체를 안 보고 바로 `/matching`으로 리다이렉트(관심사 등 나중에 미완성이 돼도 재진입 안 함 — 그런 항목은 설정 화면 `InterestsSettingsForm`에서 편집) (자세한 내용은 위 `현재 상태` 참고)
...
## 📋 다음 작업
최초 로드맵(인증 3종 + 온보딩 + 매칭 + 연결 + 설정)에 이어 인앱 메시징 + 실시간화(폴링) + 알림 뱃지, UI 크리틱 후속조치 17건, 스태프 관리자 패널(Phase 1·2 + `apps/staff` 통합), 회원가입 최소연령 검증, 카카오 소셜 로그인/가입(콘솔 등록 + 실계정 종단 검증), 문의/신고하기(관리자 답변 포함), 유저 자유게시판, 온보딩 성격 단계 500 버그 수정 + 온보딩 재진입 로직 개선(`has_completed_onboarding`) + 설정 관심사 편집까지 완료. 다음 세션 시작 시 사용자와 함께 방향을 다시 정할 것 — 후보:
