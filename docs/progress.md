...
## 🚦 현재 상태 (마지막 업데이트: 2026-08-20)
"실제 서비스로 내놓기엔 UI가 너무 대충이다"라는 사용자 피드백으로 시작해서 여러 갈래로 이어짐: (1) impeccable 정식 크리틱으로 UI 전반 품질 문제 17개를 찾아 전부 처리, (2) "Django 기본 관리자 페이지는 실제 서비스에 못 쓴다"는 지적으로 스태프 전용 관리자 패널을 새로 구축(Phase 1 → Phase 2로 이어서 완료), (3) 관리자 REST API가 유지보수 관점에서 도메인 앱에 흩어져 있으면 안 된다는 지적으로 전용 `apps/staff` 앱으로 통합, (4) "회원가입이 성인부터인데 실제로 나이를 검증 안 한다"는 지적으로 성인인증 추가 — 카카오 로그인을 먼저 시도했으나 사업자등록 요구에 막혀 자기신고 생년월일 검증으로 전환, (5) "카카오 REST API 키가 아깝다"는 지적으로 같은 앱을 성인인증과 무관한 카카오 소셜 로그인/가입 편의 기능으로 재활용 — 콘솔 설정(Redirect URI·Client Secret·동의항목)까지 실제로 끝내서 실 계정으로 가입 완료까지 라이브 검증됨, (6) "관리자에게 남기는 문의 창이 있으면 좋겠다"는 요청으로 유저→관리자 문의/신고/건의 기능 신규 구축, 이어서 "관리자가 댓글도 달 수 있어야 하는거 아니냐"는 지적으로 답변 기능까지 바로 추가, (7) "유저들끼리 게시글을 올려 소통할 수 있게 해달라"는 요청으로 카테고리별 자유게시판(글+댓글) 신규 구축, (8) 라이브 테스트 중 발견한 온보딩 성격 단계 500 버그 수정을 계기로 "관심사 같은 선택적 항목 때문에 로그인마다 마법사를 처음부터 다시 태우는 게 번거롭다"는 지적을 받아 온보딩 재진입 로직과 설정 화면 관심사 편집을 새로 정리, (9) 배포 직후 "이미 카드 만들었으면 바로 로그인돼야 하는데 다시 마법사가 뜬다"는 재현 리포트로 (8)의 백필 누락 버그를 발견/수정, 이어서 "첫 카드 만들 때 성별과 나이는 필수로 선택하게 해달라"는 요청으로 온보딩 프로필 단계의 성별/생년월일 필수 검증을 추가, (10) "회원가입 때 이미 생년월일을 받는데 온보딩에서 또 입력받으면 다르게 입력해서 악용할 수 있다"는 지적으로 생년월일을 가입 시 1회만 받고 이후 온보딩·설정 어디서도 수정 불가하게 완전히 잠금, (11) "나이 인증 API가 유료라면 직접 구현하는 게 가능하냐"는 질문에 "본인확인기관을 거쳐야 해서 개인이 우회할 방법이 없다"고 답하는 과정에서 "그럼 이메일이라도 실제 사용하는 주소인지 무료로 확인하자"는 방향으로 좁혀져, 회원가입 자체를 이메일 인증(6자리 코드) 완료 후로 막는 기능 신규 구축, (12) "다음 작업이나 고도화할 게 남았는지 확인해달라"는 요청으로 새 기능 없이 품질만 훑는 세션 진행 — 보안 재점검(마지막 점검 이후 74커밋치 신규 공격 표면: 카카오 OAuth·이메일 인증 공개 엔드포인트·게시판·스태프 API 통합) → `/code-review`(같은 범위, high 강도)로 발견한 10개(보안 1·기능버그 4·접근성/성능 2·구조적 리팩토링 3) 전부 순서대로 수정.

**코드 품질 세션 — 보안 재점검 + 코드 리뷰 10건 전부 수정** — 배포/새 기능 작업이 아니라 지금까지 쌓인 코드를 훑어 굳히는 세션. 마지막 보안 점검(8/12) 이후 74개 커밋이 쌓였는데 그사이 카카오 OAuth·이메일 인증 공개 엔드포인트·게시판 공개 작성·스태프 API 통합처럼 공격 표면이 꽤 늘어난 상태라 그 범위로 `/security-review`를 다시 돌렸고(발견 없음, 권한 체크 일관성 확인만), 이어서 같은 범위로 `/code-review`(high)를 돌려 나온 10개를 우선순위 순서대로 전부 고쳤다:
- **[보안] 카카오 소셜 가입 이메일 인증 우회** — `KakaoSignupCompletionView`가 제출된 email을 세션의 `kakao_pending_email`(카카오가 실제로 준 값)과 비교하지 않고 그대로 계정 생성에 써서, 프론트의 `disabled` 입력창을 API 직접 호출로 우회하면 임의의 미검증 이메일로 가입 가능했던 구멍 — 방금 만든 이메일 인증 게이트가 카카오 경로에서 무력화되는 문제. `KakaoSignupCompletionSerializer.validate()`에서 카카오가 이메일을 줬으면 일치 검증, 안 줬으면(동의 거부) 일반 회원가입과 동일하게 `is_recently_verified()` 게이트 통과를 요구하도록 수정. 회귀 테스트 3개.
- **[버그] `page_size` 쿼리 파라미터 무시** — `DEFAULT_PAGINATION_CLASS`가 기본 `PageNumberPagination`이라 `page_size_query_param`이 없어서, 프론트가 보내는 `?page_size=100~200`(댓글·관심사 목록을 사실상 전체 요청)이 전부 무시되고 `PAGE_SIZE=20`으로 잘렸다 — 댓글 21개 이상인 글이 일부만 보이고, 관심사 21개 이상인 유저는 설정 화면에서 이미 추가한 항목이 또 추가 가능한 것처럼 떠서 클릭하면 400 에러. `config/pagination.py`에 `DefaultPagination`(max_page_size=200) 신규.
- **[버그] `ConfirmButton` 무응답** — 스태프의 정지/삭제/상태변경 등 파괴적 액션(6개 화면 재사용)이 실패해도(403·네트워크 오류) `try/finally`만 있고 `catch`가 없어서 버튼이 조용히 idle로 되돌아가고 아무 피드백이 없었다. 인라인 에러 메시지를 보여주고 실패 시엔 idle로 안 돌려서 재시도 가능하게 수정.
- **[버그] 게시판 글/댓글 삭제 에러 처리 없음** — `handleDeletePost`/`handleDeleteComment`가 try/catch 없이 호출돼 실패 시(스태프가 이미 강제삭제 등) 글 삭제는 확인 버튼이 멈추고, 댓글 삭제는 버튼만 원상복귀되고 정작 안 지워져 아무 일도 없었던 것처럼 보였음.
- **[버그] 이메일 인증 코드 발송 SMTP 예외 미처리** — 개발 환경이 실제 SMTP로 바뀐 뒤(직전 세션) `generate_and_send_code`의 `send_mail`이 예외 처리 없이 호출돼서, SMTP 실패 시 회원가입 첫 단계가 그대로 500으로 죽었다. `EmailSendError` 신규 → 503으로 변환, 실패한 시도의 레코드는 삭제해 재전송 쿨다운(60초)에 안 걸리게 함.
- **[접근성] `SignupForm` 2단계 폼의 `aria-describedby`가 없는 id를 가리킴** — 2단계 재구성(직전 세션) 후 닉네임/생년월일/비밀번호 등 4개 필드가 전부 이메일 인증 에러용 `errorId`를 참조했는데, 그 `<p>`는 `emailPhase`가 `verified`가 되면 렌더링 자체가 안 돼서 회원가입 제출 실패 시 스크린리더가 에러를 못 찾았다. 폼 제출 에러 전용 `formErrorId`로 분리.
- **[성능] 게시판 목록 N+1 쿼리** — `comment_count`/`posts_count`가 `SerializerMethodField`로 행마다 별도 COUNT 쿼리를 날려서 글 50개 목록에 51개 쿼리. 뷰의 queryset에 `annotate(Count(...))` 추가(소비자+스태프 뷰셋 4곳), 시리얼라이저는 annotate 값이 있으면 쓰고 없으면(생성 직후 응답처럼 annotate를 안 거친 인스턴스) 기존 방식으로 폴백하는 하이브리드로 ModelViewSet의 create/update 경로를 안 깨뜨림. BoardCategory는 `Count` annotate가 `Meta.ordering`을 무시시키는 Django 특성 때문에 생긴 `UnorderedObjectListWarning`도 같이 해결.
- **[구조] 스태프 뷰셋 9개의 `IsAdminUser` 중복 선언** — 전역 기본값이 `IsAuthenticated`라 새 스태프 엔드포인트 추가 시 이 한 줄을 깜빡하면 조용히 일반 유저에게 열리는데 잡아줄 테스트가 없었음. `StaffPermissionMixin` 공유 베이스로 통합 + 라우터에 등록된 모든 뷰셋을 전수 검사하는 가드 테스트 신규(상속을 일부러 빼서 실제로 잡히는 것까지 확인).
- **[구조] 매칭 요청 취소 규칙 중복 구현** — "완료된 요청은 취소 불가" 규칙이 Django admin 일괄 액션과 스태프 API 단건 액션에 각각 다른 코드로 구현돼 있어 나중에 규칙이 바뀌면 한쪽만 고치고 놓칠 위험. `MatchingRequest` 모델에 `can_be_cancelled`(단건)/`cancellable_queryset()`(대량) 단일 소스 추가.
- **[구조] 스태프 화면 6개의 목록/페이지네이션 보일러플레이트 중복** — 목록 state·에러 처리·필터 변경 시 재조회·로딩/빈 목록/에러 렌더링·이전/다음 푸터가 6개 화면(~1470줄)에 거의 그대로 복붙, 경쟁 상태 방지 방식도 화면마다 달랐음(cancelled 플래그 vs eslint-disable). `usePaginatedList` 훅(경쟁 상태 방지 통일, `setData`/`refresh` 제공) + `StaffListStatus`/`StaffPagination` 공용 컴포넌트로 추출, 6개 화면 전부 교체(필터 state는 화면마다 달라서 그대로 둠). 순감소 158줄.
- 매 항목마다 회귀 테스트 추가 후 전체 스위트 재실행 — 백엔드 300→307개, 프론트 22→27개까지 순증하며 전부 통과 유지. `check-all.sh`(포맷팅/린트/Django check)·`tsc`·`oxlint` 매 커밋마다 통과 확인. 커밋 10개로 분리(항목당 1개).
- **후속: 이 세션에서 만든 코드 자체를 다시 `/code-review`로 점검**(`95fd8fe..HEAD`, high) — 새로 발견 2건 모두 수정. (1) `usePaginatedList`의 `refresh()`가 deps effect와 달리 경쟁 상태 방어(cancelled 플래그)가 없어서, 삭제 후 `refresh()`를 부른 직후 페이지를 옮기면 더 먼저 나간 `refresh()` 응답이 더 나중에 나간(진짜 최신) deps 요청 결과를 덮어쓸 수 있었다 — effect와 `refresh()`가 공유하는 요청 번호 카운터로 교체해 "가장 나중에 발급된 요청"만 반영하도록 수정, 경쟁을 그대로 재현하는 테스트로 수정 전 코드가 실제로 실패하는 것까지 확인. (2) `StaffBoardScreen.handleDeleteComment`의 `loadComments`가 다른 재조회 함수들과 달리 자체 에러 처리가 없어서, 댓글 삭제 자체는 성공했는데 재조회만 실패하면 삭제가 실패한 것처럼 오보고됐다 — 자체 에러 처리 추가 + 무관한 두 재조회를 `Promise.all` 병렬 처리로 변경. 프론트 테스트 28개로 순증, 전부 통과. 이 두 커밋까지 포함해 **총 커밋 12개, 아직 PR 전** — 다음에 올릴 것.

**회원가입 이메일 인증 (신규) — 실제 사용 중인 이메일인지 확인 후에만 가입 허용** — 카카오 `age_range` 실 신원인증이 사업자등록에 막힌 상황에서 "유료 나이 인증 API를 직접 구현할 수 있냐"는 질문이 나왔고, 답은 "법으로 지정된 소수 본인확인기관을 거쳐야 해서 개인이 우회 구현할 방법 자체가 없다"였음. 대신 나이는 못 증명해도 "이메일이 실제 접근 가능한 주소인지"는 기존 SMTP 인프라(비밀번호 재설정 이메일에 이미 쓰던 것)로 무료 확인 가능해서, **회원가입 자체를 이메일 인증 완료 후로 막기**로 함(사용자 확정 — 온보딩 재입력만 막는 게 아니라 가입 자체를 게이트):
- 신규 `EmailVerification` 모델 — 아직 계정이 없는 이메일이 대상이라 `User`와 FK로 안 엮이고 이메일 문자열이 식별자. 6자리 코드를 `User.password`와 같은 방식(`make_password`)으로 해싱 저장, 10분 만료·60초 재전송 쿨다운·5회 시도 제한.
- `UserViewSet`에 `request_email_verification`/`confirm_email_verification` 액션 추가(`password_reset`과 동일 패턴, `AllowAny`). 이미 가입된 이메일은 코드 요청 시점에 미리 거부해 가입 마지막 단계에서야 아는 것 방지.
- `UserCreateSerializer.validate()`에 게이트 추가 — 최근 1시간 이내 인증된 이메일이 아니면 400. 카카오 소셜 가입은 대상 아님(카카오 OAuth 자체가 이미 이메일 소유를 보증하는 별도 신뢰 경로).
- `SignupForm.tsx`를 2단계 폼으로 재구성 — 이메일 인증(코드 받기 → 6자리 입력 → 확인) 블록을 별도 `<div>`로 분리해 회원가입 `<form>`과 섞이지 않게 하고(Enter 키 오작동 방지), 인증 완료 전까지 닉네임/생년월일/비밀번호 필드 자체를 렌더링 안 함. 인증 완료 시 기존 `auth-success__badge`(틸 pill)를 재사용해 "인증 완료" 표시.
- 백엔드 회귀 테스트 11개 신규(코드 발송·중복 이메일 거부·재전송 쿨다운·정상confirm·오답 시도 증가·시도 초과·만료 코드·인증 미완료 가입 거부 등), 전체 스위트 300개 통과. 라이브 브라우저로 전체 플로우(코드 발송 → 오답 → 정답 → 인증 완료 배지 → 회원가입 → 로그인 → 온보딩 이동)와 API 직접 호출 우회 시도 차단(curl)까지 종단 검증 완료.
- **후속: 개발 환경에서도 실제 이메일 발송** — `dev.py`가 `EMAIL_BACKEND`를 console로 하드코딩해놔서 로컬에서는 인증 코드가 터미널에만 찍히고 실제 메일함으론 안 갔음("콘솔로 오면 무슨 소용이냐"는 사용자 피드백). `prod.py`와 같은 방식으로 `.envs/.env.dev`에 `EMAIL_HOST_USER`/`EMAIL_HOST_PASSWORD`가 채워지면 실제 SMTP(Gmail)로, 비어있으면 console로 남는 조건부 분기로 변경. 개인 Gmail 앱 비밀번호로 실제 발송 확인 후, "개인 메일 대신 프로젝트 전용 발신 계정을 쓸 수 있냐"는 질문에 답변 — Gmail SMTP는 인증 계정과 다른(가상의) 발신 주소를 거부하므로 완전 가상 이메일은 불가능하지만, 전용 구글 계정을 새로 만들어 그 계정으로 인증하는 건 가능(다음에 진행 예정).
...
전체 백엔드 테스트 스위트 307개(코드 품질 세션에서 추가된 카카오 이메일 검증 3개 + N+1 방지 쿼리카운트 1개 + SMTP 예외 처리 1개 + 스태프 권한 가드 1개, 이메일 인증 11개 + 생년월일 잠금 2개 + 온보딩 재진입 로직 2개 + 성격 upsert 3개, 유저 자유게시판 36개, 문의/신고하기 23개, 카카오 소셜 로그인/가입 11개 포함) 통과. 프론트엔드 28개(코드 품질 세션에서 추가된 ConfirmButton 2개 + usePaginatedList 6개 포함) 통과.
...
- 커밋 상태: 유저 자유게시판 + 성격 단계 500 버그 수정 + 온보딩 재진입 로직/설정 관심사 편집 + has_completed_onboarding 백필 버그 수정 + 온보딩 성별/생년월일 필수 선택 강제 + 생년월일 이중 입력 구멍 잠금까지는 PR #8로, 프로젝트 스킬 메모 문서는 PR #9로, 회원가입 이메일 인증(백엔드+프론트엔드) + 개발 환경 실제 SMTP 발송 전환은 PR #10으로 `origin/main`에 전부 머지 완료. `docs/skills-memo` 브랜치는 머지 후 로컬·원격 모두 삭제함. **코드 품질 세션(보안 재점검 + 코드 리뷰 10건 수정, 커밋 10개)은 로컬 `feature`에 커밋까지만 하고 아직 푸시 전** — 다음에 푸시 + PR로 올릴 것
...
- [x] 회원가입 이메일 인증 — 이메일이 실제 사용 중인 주소인지 확인 후에만 가입 허용. `EmailVerification` 모델(6자리 코드, `make_password`로 해싱, 10분 만료·60초 재전송 쿨다운·5회 시도 제한), `UserCreateSerializer.validate()`가 최근 1시간 이내 인증 여부로 게이트. `SignupForm.tsx`가 이메일 인증(코드 받기→확인) → 나머지 필드(닉네임/생년월일/비밀번호) 2단계로 재구성, 인증 전엔 나머지 필드 자체를 렌더링 안 함. 카카오 소셜 가입은 대상 아님(OAuth가 이미 이메일 소유 보증) (자세한 내용은 위 `현재 상태` 참고)
...
- [x] 코드 품질 세션 — 새 기능 없이 보안 재점검(`/security-review`, 발견 없음) + 코드 리뷰(`/code-review` high, 10건) 전부 수정: 카카오 소셜 가입 이메일 인증 우회(보안), `page_size` 쿼리 무시 페이지네이션 버그, `ConfirmButton`/게시판 삭제 에러 처리 없음, 이메일 발송 SMTP 예외로 인한 500, `SignupForm` `aria-describedby` 접근성 회귀, 게시판 목록 N+1 쿼리, 스태프 `IsAdminUser` 중복(공유 베이스 + 라우터 전수 검사 가드 테스트로 정리), 매칭 요청 취소 규칙 중복(모델로 통합), 스태프 화면 6개 목록/페이지네이션 보일러플레이트 중복(`usePaginatedList` 훅으로 추출) (자세한 내용은 위 `현재 상태` 참고)

---

## 📋 다음 작업
최초 로드맵(인증 3종 + 온보딩 + 매칭 + 연결 + 설정)에 이어 인앱 메시징 + 실시간화(폴링) + 알림 뱃지, UI 크리틱 후속조치 17건, 스태프 관리자 패널(Phase 1·2 + `apps/staff` 통합), 회원가입 최소연령 검증, 카카오 소셜 로그인/가입(콘솔 등록 + 실계정 종단 검증), 문의/신고하기(관리자 답변 포함), 유저 자유게시판, 온보딩 성격 단계 500 버그 수정 + 온보딩 재진입 로직 개선(`has_completed_onboarding`) + 설정 관심사 편집 + 생년월일 이중 입력 구멍 잠금 + 회원가입 이메일 인증 + 코드 품질 세션(보안 재점검 + 코드 리뷰 10건 수정)까지 완료. E2E 테스트 자동화는 사용자 판단으로 후보에서 제외("결국 브라우저로 확인해야 하는 건 매한가지"). 다음 세션 시작 시 사용자와 함께 방향을 다시 정할 것 — 후보:
- [ ] 코드 품질 세션 커밋 12개 푸시 + PR (사용자가 직접 진행하기로 함)
- [ ] 배포 준비 (prod 빌드 점검, 환경변수 정리)
- [ ] 이메일 발송용 전용 구글 계정으로 전환 (지금은 개인 Gmail 앱 비밀번호로 SMTP 발송 중 — 전용 계정을 새로 만들어 그걸로 인증하면 가능하다고 확인해둔 상태, 다음에 진행 예정)
