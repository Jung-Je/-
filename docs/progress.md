...
- 커밋 상태: 문의/신고하기(`apps/support` 신규, 관리자 답변 기능 포함) + 카카오 소셜 로그인/가입 후속 조치 + README 정리까지는 PR #7로 `origin/main`에 이미 머지 완료(PR #6에 이어). **유저 자유게시판(`apps/board` 신규) + 성격 단계 500 버그 수정 + 온보딩 재진입 로직/설정 관심사 편집(+ 이 문서 업데이트), 총 8커밋은 로컬 `feature` 브랜치에만 있고 아직 푸시 전** — 다음에 PR로 올릴 것
...
- [x] 온보딩("내 카드 만들기") 3단계 마법사 (`features/onboarding/`) — 프로필(성별/생년월일/지역/자기소개) → 성격(MBTI/성향 3개/가치관, 전부 선택) → 관심사(카테고리별 선택, 1개 이상 필수) → `check_profile_completion` 호출. 매칭 가중치 3색 체계(관심사=코랄 50%·성격=바이올렛 30%·위치=틸 20%, DESIGN.md)를 각 단계 카드 윗선 색에 반영. `User.has_completed_onboarding`(한 방향 래치) 값이 true인 유저는 마법사 자체를 안 보고 바로 `/matching`으로 리다이렉트(관심사 등 나중에 미완성이 돼도 재진입 안 함 — 그런 항목은 설정 화면 `InterestsSettingsForm`에서 편집) (자세한 내용은 위 `현재 상태` 참고)
