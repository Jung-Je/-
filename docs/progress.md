...
사용자 제보로 실사용 버그 2개 발견/수정:

1. **비밀번호 복잡도 미검증** — Django 기본 `AUTH_PASSWORD_VALIDATORS`(길이/공통 비밀번호/전부 숫자 여부)만으로는 영문+숫자 조합만으로도 회원가입·비밀번호 변경·재설정이 전부 통과됐음(특수문자 요구가 실제로는 어디서도 강제되지 않고 있었음). `apps/users/validators.py`에 `PasswordComplexityValidator`(영문+숫자+특수문자 모두 요구)를 만들어 `AUTH_PASSWORD_VALIDATORS`에 등록. 프론트 3개 비밀번호 입력 화면(회원가입/비밀번호 재설정/설정의 비밀번호 변경)에 힌트 문구도 추가. 브라우저로 실제 거부 메시지("비밀번호는 영문, 숫자, 특수문자를 모두 포함해야 합니다.")와 통과 케이스 둘 다 확인.
2. **매칭 시작 시 "가상 매칭 티켓" 중복 생성** — "매칭 시작" 버튼 클릭 한 번이 실제로는 서버에 POST가 두 번(수 초 간격) 도달해 결과 없는 `MatchingRequest`가 중복으로 남는 사례를 Django 서버 로그(요청 타임스탬프)로 직접 확인. 프론트는 이미 제출 중 버튼을 비활성화하지만 그걸로 완전히 막히지 않는 경우가 실제로 있었음(정확한 클라이언트 측 트리거는 간헐적이라 특정 못함). 근본 원인과 무관하게 확실히 막도록 백엔드에 방어 로직 추가 — `MatchingRequestViewSet.create()`가 같은 사용자가 5초 이내에 만든 요청이 있으면 새로 만들지 않고 기존 것을 200으로 그대로 반환(`apps/matching/views.py`). curl로 동일 세션에서 연속 POST를 재현해 두 번째 요청이 새 레코드를 안 만들고 기존 id를 돌려주는 것까지 확인.

메시징 실시간화(폴링 기반)와 인앱 메시징·프론트엔드 Docker화는 이전 세션에 완성 — 자세한 내용은 `완료된 기능` 섹션 참고.

- 커밋 상태: 이번 버그 수정 2건은 아직 커밋 전(사용자가 "커밋해줘"라고 하면 진행)
