...
설정 화면 완성 — 프로필 편집(이름/성별/생년월일/지역/자기소개/매칭 노출 토글), 비밀번호 변경(세션 유지), 계정(로그아웃/회원 탈퇴 — "탈퇴" 타이핑 확인). 이 작업 중 화면들(매칭/연결/설정) 사이를 오갈 방법이 URL 직접 입력 말고는 없었던 것도 발견해 공용 `AppNav`를 추가해 세 화면에 공통으로 붙임. 프로필 편집을 위해 `AuthUser` 타입도 `/users/me/`가 실제로 주는 필드까지 넓힘. 브라우저로 프로필 저장/비밀번호 변경(재로그인 불필요 확인)/탈퇴(DB 반영 확인)까지 전부 검증 완료.

이걸로 `docs/progress.md`가 애초에 잡았던 로드맵(로그인→회원가입→비밀번호 재설정→온보딩→매칭→연결→설정)이 전부 끝났다. 다음 세션은 새 기능/개선 방향을 다시 정해서 시작.

- 커밋 상태: 설정 화면이 아직 커밋 전
...
## 📋 다음 작업
최초 로드맵(인증 3종 + 온보딩 + 매칭 + 연결 + 설정)을 전부 완료함. 다음 세션 시작 시 사용자와 함께 다음 방향을 다시 정할 것 — 후보:
- [ ] **인앱 메시징** — 지금은 연결 요청이 수락되면 그걸로 끝, 두 사용자가 실제로 연락할 방법이 앱 안에 전혀 없음(확인됨: `UserDetailSerializer`에 이메일 등 연락처 필드 자체가 없고, 수락 시엔 이메일 알림 한 통만 감). 새 백엔드 모델(대화방/메시지) + 프론트 채팅 화면이 필요한 제일 큰 다음 기능. ACCEPTED 상태인 연결끼리만 대화 가능하도록 설계
