 ...
 *
 * 매칭/연결 탭의 배지는 "안 본 매칭 결과 수"/"응답 안 한 받은 연결 요청
 * 수"를 보여준다 — 둘 다 이미 있던 is_viewed/PENDING 상태를 그대로
 * 반영하는 것이라, 결과를 펼치거나(매칭) 요청에 응답하면(연결) 자연히
 * 줄어든다.
 */
export function AppNav() {
  const { unviewed_matching_results, pending_connection_requests } = useNotificationSummary()

  const links = [
    { to: '/matching', label: '매칭', badge: unviewed_matching_results },
    { to: '/connections', label: '연결', badge: pending_connection_requests },
    { to: '/messages', label: '메시지', badge: 0 },
